...
## Changing inequality in victimization
- Crime has fallen a lot in the last few decades
- Victimization is unequally distributed
- How has victimization inequality changed over the period of falling crime?
...
[^2]: I think these numbers are coefficients from a Negative Binomial regression model? It's possible that they are actually predicted counts - the paper isn't super clear
...
[aside]: (In that toy example, the fix is actually just to model all the years together with an interaction effect - but that won't work so well in more complex cases)
...
- There is, it turns out, statistical theory on confidence intervals for ratios and how to calculate them etc [@franzRatiosShortGuide2007] (TLDR: it's complicated)
- But I didn't need to know about these! 
- The 'rage hulk' (h/t [Quantitude](https://quantitudepod.org/?s=rage+hulk)) method of letting computer go brrr and applying the method hundreds of times identified the problem and gave calibrated uncertainty estimates - even though they were useful for spotting a problem rather than clear communication
